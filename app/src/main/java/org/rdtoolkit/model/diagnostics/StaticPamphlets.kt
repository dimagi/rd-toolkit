...
class StaticPamphlets(val context : Context) : PamphletSource {
    override fun getMatchingPamphlets(category: String, tags: List<String>): List<Pamphlet> {
        ...
        if (category.equals("reference") && tags.contains("meriscreen_pf_pv")) {
            var folioContext = AssetFolioContext("bootstrapped/meriscreen_pf_pv/reference", context.assets)
            var mediaContext = ZipStreamFolioContext(folioContext, "media.zip")
            val folio = parseFolio(folioContext.spool("folio.json"), mediaContext)
            folio.setLocale(locale.language)
            response.add(folio.getPamphlet())
        ...
        if (category.equals("interpret") && tags.contains("meriscreen_pf_pv")) {
            var folioContext = AssetFolioContext("bootstrapped/meriscreen_pf_pv/interpret", context.assets)
            var mediaContext = ZipStreamFolioContext(folioContext, "media.zip")
            val folio = parseFolio(folioContext.spool("folio.json"), mediaContext)
            folio.setLocale(locale.language)
            response.add(folio.getPamphlet())
        ...
        if (category.equals("interpret") && tags.contains("parascreen_pan_pf")) {
            var folioContext = AssetFolioContext("bootstrapped/parascreen_pan_pf/interpret", context.assets)
            var mediaContext = ZipStreamFolioContext(folioContext, "media.zip")
            val folio = parseFolio(folioContext.spool("folio.json"), mediaContext)
            folio.setLocale(locale.language)
            response.add(folio.getPamphlet())
